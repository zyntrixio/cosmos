...
def test_status_change_mangled_json(test_client: "TestClient", setup: SetupType) -> None:
    ...
    resp = test_client.post(
        f"{settings.API_PREFIX}/campaigns/{retailer.slug}/campaigns/status-change",
        data=b"{",
...
def test_status_change_invalid_retailer(test_client: "TestClient", campaign: "Campaign") -> None:
    ...
    resp = test_client.post(
        f"{settings.API_PREFIX}/campaigns/{bad_retailer}/campaigns/status-change",
        json=payload,
        headers=auth_headers,
    )
...
def test_status_change_campaign_does_not_belong_to_retailer(
    ...
    resp = test_client.post(
        f"{settings.API_PREFIX}/campaigns/{second_retailer.slug}/campaigns/status-change",
        json=payload,
        headers=auth_headers,
    )
